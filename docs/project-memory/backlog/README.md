...
| B-029 | Numbered list items all render as "1." when separated by blank lines | In processTextLines(), currentList is reset to null on any blank line. Claude often separates list items with blank lines (streaming markdown), causing each item to start a new <ol> from 1. | medium | Open |
| B-030 | Mobile sidebar stays open after selecting a conversation | At ≤600px, clicking a history entry loads the conversation but leaves the sidebar overlay open. User must manually close via × button. Should auto-close on item select. | medium | Open |
| B-031 | User message bubbles don't render inline markdown | User messages use textContent so **bold**, *italic*, `code` appear as raw asterisks/backticks. Claude bubbles render markdown fully — inconsistent UX. Low priority since typing markdown in chat is uncommon. | low | Open |
