...
| B-017 | Model selector truncates to "Sonnet — b..." on mobile | At 400px the model selector is capped at max-width:100px showing "Sonnet — b..." — ambiguous. Should use short labels (Haiku, Sonnet, Opus) on mobile or increase max-width. | low | Open |
| B-018 | Copy button appears on error bubbles — copies error text | When a stale 404 conversation loads, the error message bubble gets a Copy button. Copying it puts the error string ("This conversation is no longer available...") on clipboard — useless and confusing. Copy button should only appear on real Claude responses. | low | Open |
