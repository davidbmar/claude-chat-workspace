...
| B-046 | Unordered list renders as plain divs instead of ul/li | Lines starting with `- ` produce standalone div.bubble elements with a • character instead of proper `<ul><li>` semantic markup. Numbered lists render as proper ol/li — inconsistency. | high | Open |
| B-047 | Horizontal rule (`---`) nearly invisible | `<hr>` border-color matches background (~rgb(46,51,71) on rgb(30,34,53)) — effectively invisible on dark theme. Needs lighter color e.g. rgb(74,85,104). | low | Open |
| B-048 | Tab order dominated by sidebar delete buttons | 17 sidebar delete (×) buttons precede the message input in tab order. Keyboard user must press Tab 21 times to reach the textarea. Fix: tabindex="-1" on delete buttons, add skip-nav, or reorder DOM. | medium | Open |
| B-049 | Copy button and New Chat button missing aria-label | Both use `title` attribute only — screen readers don't reliably announce title. Need explicit `aria-label="Copy message"` and `aria-label="Start a new conversation"`. | low | Open |
